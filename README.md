...
```bash
# A. Launch the Enterprise React Workbench (Recommended for Defence/PSU demos)
python3 src/server.py
# Opens at http://localhost:8000

# B. (Alternative) Launch the Streamlit Rapid-Prototype dashboard
streamlit run ui/app.py
# Opens at http://localhost:8501

# C. Launch Microsoft Data Formulator (Interactive Spreadsheet & Charting Studio)
...
1. Launch packet capture on the physical host interface:
   ```bash
   sudo tcpdump -i any "not host 127.0.0.1 and not port 8501" -v
   ```
